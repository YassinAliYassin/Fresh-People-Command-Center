...
/**
 * Sanitize a URL or token from env var.
 * Strips "Y\n" prefix and other garbage from CLI confirmations.
 * Returns clean string or empty if no valid content.
 */
export function sanitizeEnvValue(value) {
  if (!value) return '';
  let v = String(value).trim();
  // Strip "Y\n" or "yes\n" or "Y\r\n" prefix from interactive CLI answers
  v = v.replace(/^(Y|yes|YES)\s*\r?\n/i, '');
  // Strip any whitespace, newlines, tabs
  v = v.replace(/[\s\r\n\t]+/g, '');
  return v;
}
